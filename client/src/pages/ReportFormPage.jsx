...
const ReportFormPage = () => {
    ...
    if (isEdit) {
      ...
      getReportByIdApi(id)
        .then((res) => {
          ...
          setTanggalM1(r.tanggal_m1 || '');
          setTanggalM2(r.tanggal_m2 || '');
          setTanggalM3(r.tanggal_m3 || '');
          setTanggalM4(r.tanggal_m4 || '');
  ...
  const handleStationChange = async (stationId) => {
    ...
    try {
      ...
      points.forEach((p) => {
        map[p.id] = {
          m1_berfungsi: '-', m1_terbackup: '-',
          m2_berfungsi: '-', m2_terbackup: '-',
          m3_berfungsi: '-', m3_terbackup: '-',
          m4_berfungsi: '-', m4_terbackup: '-',
        };
      });
      setDetailsMap(map);
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    if (!window.confirm('Apakah Anda yakin ingin mengirim laporan ini untuk di-review? Data tidak bisa diubah setelah di-submit.')) {
      return;
    }
    ...
    <div className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <h1 className="page-title">{isEdit ? 'Edit Form Monitoring CCTV' : 'Input Form Monitoring CCTV'}</h1>
          <p className="page-subtitle">Isi data pemeriksaan CCTV mingguan (M1-M4)</p>
        </div>
      ...
      <div className="card" style={{ marginBottom: '24px' }}>
        <h3 style={{ fontSize: '15px', fontWeight: '700', marginBottom: '16px' }}>Informasi Periode & Stasiun</h3>
        <div className="form-row">
          ...
          <div className="form-group">
            ...
            <select className="form-control" value={bulan} onChange={(e) => setBulan(e.target.value)} disabled={isEdit}>
              {MONTHS.map((m, idx) => (
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-control" value={noRef} onChange={(e) => setNoRef(e.target.value)} />
          </div>
        </div>

        {/* TANGGAL PELAKSANAAN MINGGUAN */}
        <h3 style={{ fontSize: '14px', fontWeight: '600', margin: '16px 0 12px', color: 'var(--text-secondary)' }}>
          Tanggal Pelaksanaan Mingguan
        </h3>
        <div className="form-row">
          <div className="form-group">
            <label className="form-label">Tgl M1</label>
            <input type="date" className="form-control" value={tanggalM1} onChange={(e) => setTanggalM1(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Tgl M2</label>
            <input type="date" className="form-control" value={tanggalM2} onChange={(e) => setTanggalM2(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Tgl M3</label>
            <input type="date" className="form-control" value={tanggalM3} onChange={(e) => setTanggalM3(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Tgl M4</label>
            <input type="date" className="form-control" value={tanggalM4} onChange={(e) => setTanggalM4(e.target.value)} />
          </div>
        </div>
      </div>
                  ...
                  <tr key={point.id}>
                    ...
                    <td>
                      <StatusToggle
                        ...
                        onChange={(val) => handleToggle(point.id, 'm1_berfungsi', val)}
                      />
                    </td>
                    <td>
                      <StatusToggle
                        value={detail.m1_terbackup || '-'}
                        onChange={(val) => handleToggle(point.id, 'm1_terbackup', val)}
                      />
                    </td>
                    ...
                    <td>
                      <StatusToggle
                        ...
                        onChange={(val) => handleToggle(point.id, 'm2_berfungsi', val)}
                      />
                    </td>
                    <td>
                      <StatusToggle
                        value={detail.m2_terbackup || '-'}
                        onChange={(val) => handleToggle(point.id, 'm2_terbackup', val)}
                      />
                    </td>
                    ...
                    <td>
                      <StatusToggle
                        ...
                        onChange={(val) => handleToggle(point.id, 'm3_berfungsi', val)}
                      />
                    </td>
                    <td>
                      <StatusToggle
                        value={detail.m3_terbackup || '-'}
                        onChange={(val) => handleToggle(point.id, 'm3_terbackup', val)}
                      />
                    </td>
                    ...
                    <td>
                      <StatusToggle
                        ...
                        onChange={(val) => handleToggle(point.id, 'm4_berfungsi', val)}
                      />
                    </td>
                    <td>
                      <StatusToggle
                        value={detail.m4_terbackup || '-'}
                        onChange={(val) => handleToggle(point.id, 'm4_terbackup', val)}
                      />
                    </td>
                  </tr>
                );
